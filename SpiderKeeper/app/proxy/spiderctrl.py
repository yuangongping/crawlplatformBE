...
from SpiderKeeper.app.schedulers.model import JobExecution, JobInstance, SpiderStatus, JobPriority
...
class SpiderAgent():
    ...
    def get_spider_list(self, project):
        ...
        if project.is_msd == '0':  # 如果是单机爬虫
            ...
            for spider_name in spider_instance_list_slave:  # 轮询从爬虫名字
                ...
                spider_instance_single_list.append(spider_instance_single)  # 将蜘蛛实例追加到列表中
        ...
        for spider_service_instance in self.spider_service_instances_slave:
            ...
            if project.is_msd == '0':
                job_execution_dict = dict(
                    [(job_execution.service_job_execution_id.split('>')[0], job_execution) for job_execution in
                     job_execution_list])
            else:
                job_execution_dict = dict(
                    [(job_execution.service_job_execution_id.split('>')[-1], job_execution) for job_execution in
                     job_execution_list])
            '''
    ...
    def deploy(self, project, egg_path_dict, is_msd):
        ...
        if is_msd == '0':  # 如果是单机爬虫从主爬虫代理中选择一个代理部署爬虫代码
            spider_service_instance = random.choice(self.spider_service_instances_slave)
            # 部署单机爬虫
    ...
    def log_url_master(self, job_execution):
        ...
        # 爬虫运行的服务器地址
        running_on = job_execution.running_on.split('>')[0]
        if project.is_msd == '0':  # 如果是单机爬虫调用从服务器日志
            # 调用主爬虫的日志
            for spider_service_instance in self.spider_service_instances_slave:
                if spider_service_instance.server == running_on:
                    log_url = spider_service_instance.log_url(
